...
from inference.model_loader import FEATURE_NAMES, load_model
from inference.schemas import RiskResponse, TransactionRequest
...
async def score_transaction(req: TransactionRequest):
    ...
    try:
        ...
        # Store in cache
        # -----------------------
        cache[key] = {"risk_score": float(risk_score), "decision": decision}
        ...
        # -----------------------
        return RiskResponse(
            risk_score=float(risk_score), decision=decision, model_version="logreg_v1"
        )

    except Exception as e:
        logger.error("error_occurred", extra={"extra_data": {"error": str(e)}})
